...
const AllSaler = () => {
  ...
  const {data: users = [], refetch} = useQuery({
      queryKey: ['users'],
      queryFn: async() =>{
          const res = await fetch('https://laptop-reseler-server-side-hazratali-pixel.vercel.app/user/saler/');
          const data = await res.json();
          const test = data.respons
          console.log(test);
          console.log(data);
          return data.respons;
      }
  ...
  const handleMakeVerify = id => {
      fetch(`https://laptop-reseler-server-side-hazratali-pixel.vercel.app/user/verify/${id}`, {
          method: 'PATCH'
      })
      .then(res => res.json())
      .then(data => {
  ...
  const handleDelete = id => {
    ...
    return (
        <div>
            <h2 className="text-3xl">All Saler List</h2>
    ...
    <tbody>
      {
        users.map((user, i) =><tr key={user._id}>
            ...
            {
              user?.userType==="Admin"? <td>{ user?.role !== 'admin' && <button className='btn btn-xs btn-success'>Admin</button>}</td> :
              <td>{ user?.role !== 'admin' && <button onClick={() => handleMakeAdmin(user._id)} className='btn btn-xs btn-error'>Make Admin</button>}</td>
            }            
            {
              user?.userFlag? <td>{ user?.role !== 'admin' && <button className='btn btn-xs btn-success'>Verified</button>}</td> :
              <td>{ user?.role !== 'admin' && <button onClick={() => handleMakeVerify(user._id)} className='btn btn-xs btn-warning'>Verify Saler</button>}</td>
            }            
